...
class CoinMarketCap 
{
    ...
    getMetadata(args = {}) 
    {
        return createRequest({
            ...
            config: this.config,
            query: sanitize(args.id, args.slug, args.symbol)
        });
    }
    ...
    getQuotes(args = {}) 
    {
        ...
        const { id, symbol } = sanitize(args.id, args.slug, args.symbol);
    ...
    getGlobal({ convert, convert_id }) 
    {
        ...
        if (convert_id instanceof Array) 
            convert_id = { convert_id: convert_id };

        if (convert && convert.convert instanceof Array) 
            convert.convert = convert.convert.join(",");

        if (convert_id && convert_id.convert_id instanceof Array) 
            convert_id.convert_id = convert_id.convert_id.join(",");

        return createRequest({
            fetcher: this.fetcher,
            url: `${this.url}/global-metrics/quotes/latest`,
            config: this.config,
            query: convert || convert_id
        });
    }
}

const sanitize = (id, slug, symbol) => 
{
    if (id && symbol || id && slug || slug && symbol)
        throw "ID, slug and symbol cannot be passed in at the same time.";

    if (!id && !slug && !symbol)
        throw "Either ID, slug or symbol is required to be passed in.";
